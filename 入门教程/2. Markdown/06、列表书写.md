...
CommonMark 和 其他一些轻量级标记语言允许您使用括号 ( `）`) 作为分隔符（例如，`1）第一项`），但并非所有支持 Markdown 的应用程序都支持这一点，因此从兼容性角度来看，这不是一个好选项。为了兼容性，请仅使用英文句点。

![Pasted image 20230121201257](https://s1.vika.cn/space/2023/03/14/1f620dff3c324ba08a423aed811c495c)
...
兼容 Markdown 的应用程序都不支持同一列表中试用不同分隔符（识别符）。为了兼容性，不要在同一列表中混用分隔符，选择 `- ` 并坚持使用

![Pasted image 20230121201611](https://s1.vika.cn/space/2023/03/14/eeaa30c205c848c9981fe4880c1582c5)
